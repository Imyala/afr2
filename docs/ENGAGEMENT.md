...
| **Daily quests** 🎯 | 3 fresh, achievable goals each day (e.g. "complete a lesson", "read a story") with gem rewards. Gives a reason to open the app *today*. | `gamify.js` → `QUEST_POOL` |
| **Achievement badges** 🏅 | 12 milestone badges (first lesson, streaks, words mastered, perfect lessons, reading, polyglot…). Long-term goals. | `gamify.js` → `ACHIEVEMENTS` |
...
| **Daily login reward** 🎁 | A gem chest that grows with consecutive logins, then resets — the classic "return tomorrow" hook. | `gamify.claimDailyReward` |
